...
export default function MainPage({ navigate }) {
  ...
  // 데이터 상태
  const [priceData, setPriceData] = useState({ golf: [], condo: [], fitness: [] });
  const [urgentData, setUrgentData] = useState({ golf: [], condo: [], fitness: [] });
  const [saleData, setSaleData] = useState({ golf: [], condo: [], fitness: [] });
  ...
  useEffect(() => {
    const loadAllData = async () => {
      try {
        ...
        // 시세표 데이터 로드
        const priceTableData = await getMainPriceTable();
        const formattedPriceData = { golf: [], condo: [], fitness: [] };
        
        Object.keys(priceTableData).forEach(category => {
          formattedPriceData[category] = priceTableData[category].map(m => ({
        ...
        // 급매 데이터 로드
        const urgentSalesData = await getMainUrgentSales();
        const formattedUrgentData = { golf: [], condo: [], fitness: [] };
        
        urgentSalesData.forEach(u => {
          if (u.memberships) {
            formattedUrgentData[u.category].push({
              name: `${u.memberships.product_name} ${u.memberships.membership_name}`,
              price: u.urgent_price.toLocaleString(),
              location: u.memberships.location
            });
          }
        });
        setUrgentData(formattedUrgentData);

        // 분양 데이터 로드
        const presalesData = await getMainPresales();
        const formattedSaleData = { golf: [], condo: [], fitness: [] };
        const emojis = { golf: '🏌️', condo: '🏨', fitness: '💪' };
        
        presalesData.forEach(p => {
          if (p.memberships) {
            formattedSaleData[p.category].push({
              productName: p.memberships.product_name,
              membershipName: p.memberships.membership_name,
              price: p.presale_price,
              location: p.memberships.location,
