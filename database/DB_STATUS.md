...
- 필요한 필드: c_id, urgent_price (선택)
- category, original_price는 자동으로 입력됨
- urgent_price를 입력하지 않으면 현재가와 동일하게 설정됨

**예시:**
```sql
-- 급매가 지정
INSERT INTO urgent_sales (c_id, urgent_price) VALUES (1, 42000);

-- c_id만 입력 (현재가와 동일하게 자동 설정)
INSERT INTO urgent_sales (c_id) VALUES (1);
```

### 3. 분양 데이터 입력 필요 (presaleDB)
- presales 테이블에 데이터 입력
- 필요한 필드: c_id, presale_price (선택)
- category, original_price는 자동으로 입력됨
- presale_price를 입력하지 않으면 현재가와 동일하게 설정됨

**예시:**
```sql
-- 분양가 지정
INSERT INTO presales (c_id, presale_price) VALUES (1, 55000);

-- c_id만 입력 (현재가와 동일하게 자동 설정)
INSERT INTO presales (c_id) VALUES (1);
```
